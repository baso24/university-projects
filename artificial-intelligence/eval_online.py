...
def load_test_boards_from_csv(filename="test_dataset.csv", num_boards=10):
    # read the first 'num_boards' from the test dataset
    # returns a list of tuples: (board_state_list, real_optimal_cost)
...
def evaluate_online():
    ...
    try:
        ...
        print("Error: Could not find model weights. Run train.py first.")
        return
    ...
    if not test_data:
        print("Error: Could not find test boards. Run generate_dataset.py first.")
        return
