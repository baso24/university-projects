...
def evaluate_embedding():
    ...
    print("\n" + "=" * 125)
    print("           BENCHMARK: CLASSIC vs POSITIONAL vs DELTA vs MULTI-TASK vs EMBEDDING")
    print("=" * 125)

    times_c, times_p, times_d, times_mt, times_emb = [], [], [], [], []
    nodes_c, nodes_p, nodes_d, nodes_mt, nodes_emb = [], [], [], [], []
    ...
    with torch.no_grad():
        for i, (start_board, target_cost) in enumerate(test_data):
            ...
            times_c.append(t_c); times_p.append(t_p); times_d.append(t_d); times_mt.append(t_mt); times_emb.append(t_emb)
            nodes_c.append(c_nodes); nodes_p.append(p_nodes); nodes_d.append(d_nodes); nodes_mt.append(mt_nodes); nodes_emb.append(emb_nodes)

    print(f"\n{'Metric':<32} | {'Classic A*':<16} | {'Positional':<16} | {'Delta':<16} | {'Multi-Task':<16} | {'Embedding':<16}")
    print("-" * 125)
    print(f"{'Average Nodes Expanded':<32} | {sum(nodes_c)/len(nodes_c):<16.1f} | {sum(nodes_p)/len(nodes_p):<16.1f} | {sum(nodes_d)/len(nodes_d):<16.1f} | {sum(nodes_mt)/len(nodes_mt):<16.1f} | {sum(nodes_emb)/len(nodes_emb):<16.1f}")
    print(f"{'Average Execution Time (s)':<32} | {sum(times_c)/len(times_c):<16.4f} | {sum(times_p)/len(times_p):<16.4f} | {sum(times_d)/len(times_d):<16.4f} | {sum(times_mt)/len(times_mt):<16.4f} | {sum(times_emb)/len(times_emb):<16.4f}")
    print("=" * 125 + "\n")
