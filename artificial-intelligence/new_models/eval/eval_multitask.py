...
def evaluate_multitask():
    ...
    print(f"Multi-Task Benchmark starting on: {device}\n")
    ...
    mt_path = os.path.join(root_dir, "models", "multitask", "best_multitask_model.pth")
    try:
        mt_model.load_state_dict(torch.load(mt_path, map_location=device, weights_only=True))
        mt_model.eval()
        print(f"[*] Multi-Task Model loaded from {os.path.basename(mt_path)}")
    except FileNotFoundError:
        print("Error: Could not find Multi-Task model weights. Run train_multitask.py first.")
        return

    test_data = load_test_boards_from_csv(num_boards=10)
    
    print("\n" + "=" * 115)
    print("           BENCHMARK: CLASSIC vs POSITIONAL vs DELTA vs MULTI-TASK DELTA")
    print("=" * 115)

    times_c, times_p, times_d, times_mt = [], [], [], []
    nodes_c, nodes_p, nodes_d, nodes_mt = [], [], [], []
    ...
    with torch.no_grad():
        for i, (start_board, target_cost) in enumerate(test_data):
            ...
            times_c.append(t_c); times_p.append(t_p); times_d.append(t_d); times_mt.append(t_mt)
            nodes_c.append(c_nodes); nodes_p.append(p_nodes); nodes_d.append(d_nodes); nodes_mt.append(mt_nodes)

    print(f"\n{'Metric':<35} | {'Classic A*':<18} | {'Positional A*':<18} | {'Delta A*':<18} | {'Multi-Task Delta':<18}")
    print("-" * 115)
    print(f"{'Average Nodes Expanded':<35} | {sum(nodes_c)/len(nodes_c):<18.1f} | {sum(nodes_p)/len(nodes_p):<18.1f} | {sum(nodes_d)/len(nodes_d):<18.1f} | {sum(nodes_mt)/len(nodes_mt):<18.1f}")
    print(f"{'Average Execution Time (Seconds)':<35} | {sum(times_c)/len(times_c):<18.4f} | {sum(times_p)/len(times_p):<18.4f} | {sum(times_d)/len(times_d):<18.4f} | {sum(times_mt)/len(times_mt):<18.4f}")
    print("=" * 115 + "\n")
