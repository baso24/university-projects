...
from environment import GOAL_STATE_8
from search import GRID_SIZE
from environment import GOAL_STATE_15
import os
import random
import sys
...
from environment import get_neighbors, GRID_SIZE_15
...
def encode_state(board_state):
    num_tiles = GRID_SIZE_15 * GRID_SIZE_15
    coords = torch.zeros(num_tiles * 2, dtype=torch.float32)
    for pos_idx, tile_val in enumerate(board_state):
        coords[tile_val * 2] = pos_idx // GRID_SIZE_15      # row
        coords[tile_val * 2 + 1] = pos_idx % GRID_SIZE_15   # col
    return coords
...
def scramble_from_goal(num_moves):
    ...
    for _ in range(num_moves):
        neighbors = [ns for ns, _ in get_neighbors(state, GRID_SIZE_15)]
...
def compute_bellman_targets(states, target_net, device):
    ...
    for s in states:
        if s == GOAL_STATE_15:
            ...
            nbrs = [ns for ns, _ in get_neighbors(s, GRID_SIZE_15)]
